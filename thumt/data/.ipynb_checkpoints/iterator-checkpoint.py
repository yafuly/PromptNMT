...
class _BucketDSIter(IteratorBase):
    ...
    def _fill(self) -> None:
        for i in range(self._max_fill):
            ...
            max_length = max([len(item) for item in items]) # max length of (src_sent, tgt_sent, label_sent)
...
class _TokenizedAdviceDSIter(IteratorBase):

    def __init__(self, dataset: "Dataset", seed: int = 1, epoch: int = -1):
        self._bos = dataset.bos
        self._eos = dataset.eos
        self._tokenizer = dataset.tokenizer
        self._iterator = iter(dataset._dataset)
    ...
    @_profile("_TokenizedLineDSIter", False)
    def __next__(self) -> List[bytes]:
        val = self._tokenizer.encode(next(self._iterator), random_generator=self.random_generator, epoch=self.epoch)
...
_DATASET_TO_ITER = {
    ...
    "TokenizedAdviceDataset": _TokenizedAdviceDSIter,
...
class Iterator(IteratorBase):
    ...
    def __init__(self, dataset: "Dataset", seed: int = 1, epoch: int = -1):
        iter_class = _DATASET_TO_ITER[dataset.name]
        if iter_class == _TokenizedAdviceDSIter:
            self._iterator = iter_class(dataset, seed, epoch)
        else:
            self._iterator = iter_class(dataset)
