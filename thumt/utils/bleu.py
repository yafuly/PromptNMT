...
def bleu(trans, refs, bp="closest", smooth=False, n=4, weights=None):
    ...
    trans = [" ".join([e.decode("utf-8")  for e in t]) for t in trans]
    refs = [" ".join([e.decode("utf-8")  for e in r[0]]) for r in refs]
    # with open("/home/amax/Codes/nmt-inter-state/THUMT/tmp/tmp.hyp", 'w') as f:
    #     f.write("\n".join(trans))
    # with open("/home/amax/Codes/nmt-inter-state/THUMT/tmp/tmp.ref", 'w') as f:
    #     f.write("\n".join(refs))
    bleu = sacrebleu.corpus_bleu(trans, [refs], tokenize="none") # do not use tokenize to compute BLEU before debpe
    score = bleu.score
    return score
